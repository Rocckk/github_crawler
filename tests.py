...
class TestCrawlerWikis(unittest.TestCase):
    ...
    def test_get_urls(self):
        urls = get_urls(self.keywords, self.proxy_list, self.obj_type)
...
class TestCrawlerRepos(unittest.TestCase):
    ...
    def test_parse_repo_result_type(self):
        if self.urls:
            result = parse_repo(self.urls[0].get('url'), self.proxy_list[0])
    ...
    def test_get_extra_return_type(self):
        results = get_extra(self.urls, 'Repositories', self.proxy_list)
